...
export async function POST(request){
  ...
  try{
    ...
    const recipients=Array.isArray(body.recipients)?body.recipients.filter(r=>r?.email).slice(0,120):[];
    ...
    if(!apiKey){
      return NextResponse.json({ok:true,emailConfigured:false,status:'pending',sent:0,failed:0,recipientCount:recipients.length},{status:202,headers:{'Cache-Control':'no-store'}})
    }
    ...
    const results=await Promise.allSettled(recipients.map(recipient=>fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[testRecipient||recipient.email],subject,html:html.replace('</div></div></body></html>',`<p style="font-size:11px;line-height:1.6;color:#819087;margin:18px 0 0">Intended student: ${escapeHtml(recipient.email)}</p></div></div></body></html>`)})}).then(async response=>({ok:response.ok,status:response.status,body:await response.text()}))));
    const sent=results.filter(result=>result.status==='fulfilled'&&result.value.ok).length;
    const failed=recipients.length-sent;
    return NextResponse.json({ok:true,emailConfigured:true,status:failed?'partial':'sent',sent,failed,recipientCount:recipients.length},{headers:{'Cache-Control':'no-store'}})
  }catch{
