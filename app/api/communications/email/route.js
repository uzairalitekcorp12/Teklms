...
export async function POST(request){
  ...
  try{
    ...
    const recipients=(Array.isArray(body.recipients)?body.recipients:[])
      ...
      .map(recipient=>({id:String(recipient.id||'').trim(),name:String(recipient.name||'Student').slice(0,120),email:String(recipient.email).trim().slice(0,254)}))
      .slice(0,120);
    ...
    const targets=studentTen.map(recipient=>({name:recipient.name,email:testRecipient}));
    ...
    return NextResponse.json({ok:sent>0,configured:true,testMode:true,sent,failed,audienceCount:recipients.length,status:sent?(failed?'Partially accepted':'Accepted by provider'):'Failed',message:sent?'Email accepted for Student 10 testing.':'Email rejected. Check the Resend sender domain and recipient restrictions.'},{status:sent?200:502,headers:{'Cache-Control':'no-store'}});
  }catch{
