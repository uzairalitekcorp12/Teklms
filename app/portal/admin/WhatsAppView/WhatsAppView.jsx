...
export default function WhatsAppView(){
  ...
  const send=async()=>{
    ...
    setSending(true);
    try{
      const response=await fetch(`/api/communications/${channel==='Email'?'email':'whatsapp'}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subject,message,recipients:selectedStudents.map(({name,email,phone})=>({name,email,phone}))})});
