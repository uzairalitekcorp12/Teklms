...
export async function POST(request){
  ...
  try{
    ...
    const results=await Promise.allSettled(recipients.map(recipient=>fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[testRecipient||recipient.email],subject,html})}).then(async response=>({ok:response.ok,status:response.status,body:await response.text()}))));
