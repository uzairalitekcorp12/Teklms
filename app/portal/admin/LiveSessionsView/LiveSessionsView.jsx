...
export default function LiveSessionsView(){
  ...
  const eligible=useMemo(()=>state.students.filter(student=>(student.courseCodes||[]).includes(form.courseCode)),[state.students,form.courseCode]);
  ...
  useEffect(()=>{setSelectedIds(eligible.map(s=>s.id));setStudentSearch('')},[form.courseCode,state.students]);
  ...
  const submit=async e=>{
    ...
    sendLiveSession({title:form.title.trim(),courseTitle:course.title,courseCode:course.code,url:form.url.trim(),message:form.message.trim(),recipientIds:selectedStudents.map(s=>s.id),recipientCount:selectedStudents.length,emailStatus:emailResult.emailConfigured?(emailResult.failed?'Partially sent':'Sent'):'Pending mail setup',emailSent:emailResult.sent||0,emailFailed:emailResult.failed||0});
  ...
  return <div className="live-admin-view">
    <PageHeading eyebrow="Class communication" title="Live Sessions" copy="Share YouTube live classes with selected course students through email and their portal notification panel." hideExport secondary={<span className="live-ready-chip"><Radio/> Live link delivery</span>}/>
    ...
    <div className="live-compose-layout">
      <form className="live-compose-card" onSubmit={submit}>
        ...
        <div className="live-form-grid">
          ...
          <label className="field"><span>Session title</span><input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="e.g. Calculus revision — live class"/></label>
        ...
        <section className="recipient-section">
          <button type="button" className="recipient-title-row" onClick={()=>setShowRecipients(v=>!v)}><div><span className="recipient-icon"><Users/></span><div><b>Recipients</b><small>{selectedIds.length} of {eligible.length} students selected</small></div></div><ChevronDown className={showRecipients?'open':''}/></button>
